...
// ======================================================================
//   WTFF — Windowed Timing Frame Finder 0.6.2 (non‑binning output fix)
//   By Grimnismal & a bit of AI, vibe coded.
//   Merged: GunZ native engine + OpenMP + restored standard integer mode
//           and single‑point mode from 0.3.0.
//   Improvements:
//     - Kahan summation in standard integer mode
//     - Configurable SWEET_WEIGHT for standard analysis
//     - NULL crash protection for tiny ranges
//     - Robust deduplication (one integer FPS per rank in GunZ mode)
//     - Exact discrete simulation with overshoot (binary‑search O(log N))
//     - Correct BLOCK_METRIC_MODE handling
//     - Motion jitter analysis (intra‑block stutter) for BEST ACCURACY
//     - HIGHEST MOTION JITTER section (in animation ticks)
//     - Motion jitter shown in FASTEST/SLOWEST BLOCK lists
//     - Non‑binning mode (BINNING_MODE 0) now correctly prints full TOP_N list
// ======================================================================
...
#define TARGET_RANGE     64.0     // Allowed distance from the anchor, candidates for your optimal fps
#define TARGET_DIRECTION 1        // 0 = both sides, 1 = only ≤ anchor, 2 = only ≥ anchor
...
#define TOP_N        64            // How many distinct values to print in the list, quality of life setting
...
// 1 = on  (block‑time accuracy analysis, ref 250ms / 1140 frames)
...
typedef struct {
    ...
    double sweet;
    prec_t sweet_prec;
    double motion_jitter;
    prec_t motion_jitter_prec;
    int valid;
} BestResult;
...
// Standard sweet spot jitter weight (only used when Gunz_Fdelta_finder == 0)
// Default 1.0 gives equal weight to mean absolute error and jitter.
#define SWEET_WEIGHT 1.0

// Motion jitter weight for GunZ native best‑accuracy scoring
#define SWEET_WEIGHT_MOTION 1.0
...
static int cmp_jitter_native(const void *a, const void *b) {
    const RankResult *A = (const RankResult*)a;
    const RankResult *B = (const RankResult*)b;

    if (A->stddev > B->stddev) return -1;  // higher jitter first
    if (A->stddev < B->stddev) return 1;
...
static int cmp_motion_jitter_native(const void *a, const void *b) {
    const RankResult *A = (const RankResult*)a;
    const RankResult *B = (const RankResult*)b;

    if (A->motion_jitter > B->motion_jitter) return -1;  // higher motion jitter first
    if (A->motion_jitter < B->motion_jitter) return 1;
...
// -----------------------------------------------------------------
// GunZ native engine metric computation (exact closed‑form with binary search,
// block‑time jitter, and motion jitter)
// -----------------------------------------------------------------
static void compute_gunz_fdelta_metrics(double cfps,
                                        prec_t *out_mean_ms,
                                        prec_t *out_stddev_ms,
                                        prec_t *out_motion_jitter,
                                        prec_t *out_score)
{
    const int NJ = 21;
    double half_win = WINDOW_SIZE;
    double step = (2.0 * half_win) / (NJ - 1);

    prec_t sum_t  = (prec_t)0.0;
    prec_t sum_t2 = (prec_t)0.0;
    prec_t sum_mj = (prec_t)0.0;
    int used = 0;
    ...
    for (int j = 0; j < NJ; ++j) {
        ...
        double T = 1000.0 / f;               // exact frame time (ms)
        int d_lo = (int)floor(T);
        int d_hi = d_lo + 1;

        int L_lo = (int)(d_lo * 4.8);
        int L_hi = (int)(d_hi * 4.8);

        // Binary search for smallest N with total advance >= 1140
        int lo = 1, hi = 200000;
        while (lo < hi) {
            int mid = (lo + hi) / 2;

            // Exact total time after mid frames (timeGetTime pattern)
            double total_time = floor(mid * T + 0.5);
            int n_hi = (int)(total_time - mid * d_lo);   // number of hi deltas
            int n_lo = mid - n_hi;
            int advance = n_lo * L_lo + n_hi * L_hi;

            if (advance >= (int)GUNZ_BLOCK_FRAMES)
                hi = mid;
            else
                lo = mid + 1;
        }
        ...
        int N = lo;
        double block_ms = floor(N * T + 0.5);   // exact time when block animation ends

        // Total time according to BLOCK_METRIC_MODE
#if BLOCK_METRIC_MODE == 0
        double total_ms = block_ms + T;          // single‑block: animation + release frame
#else
        double total_ms = block_ms + 2.0 * T;   // repeat‑interval: animation + release + next‑press
#endif

        ++used;
        prec_t t_prec = (prec_t)total_ms;
        sum_t  += t_prec;
        sum_t2 += t_prec * t_prec;

        // Compute motion jitter for this jitter point
        double total_frames_time = floor(N * T + 0.5);
        int n_hi = (int)(total_frames_time - N * d_lo);
        int n_lo = N - n_hi;
        double mean_adv = (n_lo * L_lo + n_hi * L_hi) / (double)N;
        double var_adv = ( n_lo * (L_lo - mean_adv) * (L_lo - mean_adv) +
                           n_hi * (L_hi - mean_adv) * (L_hi - mean_adv) ) / (double)N;
        if (var_adv < 0.0) var_adv = 0.0;
        prec_t motion_sample = PREC_SQRT((prec_t)var_adv);
        sum_mj += motion_sample;
    }

    if (used == 0) {
        *out_mean_ms = *out_stddev_ms = *out_motion_jitter = *out_score = prec_inf();
        return;
    }

    prec_t mean_ms = sum_t / (prec_t)used;
    prec_t var_ms  = sum_t2 / (prec_t)used - mean_ms * mean_ms;
    if (var_ms < (prec_t)0.0) var_ms = (prec_t)0.0;
    prec_t stddev_ms = PREC_SQRT(var_ms);

    prec_t motion_jitter = sum_mj / (prec_t)used;   // average motion jitter in ticks

    prec_t dev = PREC_ABS(mean_ms - (prec_t)GUNZ_BLOCK_REF_MS);
    prec_t score = dev + (prec_t)GUNZ_JITTER_WEIGHT * stddev_ms +
                   (prec_t)SWEET_WEIGHT_MOTION * motion_jitter;

    *out_mean_ms        = mean_ms;
    *out_stddev_ms      = stddev_ms;
    *out_motion_jitter  = motion_jitter;
    *out_score          = score;
}
...
static void insert_up(BestResult *arr, size_t cap, size_t *cnt,
    ...
    if (*cnt < cap) {
        ...
        arr[i].motion_jitter = (double)motion_jitter_p;
        arr[i].motion_jitter_prec = motion_jitter_p;
        arr[i].valid = 1;
        (*cnt)++;
...
// Insert for highest jitter (largest stddev first)
static void insert_jitter(BestResult *arr, size_t cap, size_t *cnt,
                          double fps, prec_t avg_p, prec_t stddev_p,
                          prec_t motion_jitter_p) {
    size_t i = 0;
    while (i < *cnt && arr[i].stddev_prec > stddev_p) i++;
    if (*cnt < cap) {
        for (size_t j = *cnt; j > i; j--) arr[j] = arr[j-1];
        arr[i].fps = fps;
        arr[i].avg = (double)avg_p;       arr[i].avg_prec = avg_p;
        ...
        arr[i].valid = 1;
        (*cnt)++;
    } else if (stddev_p > arr[cap-1].stddev_prec) {
        for (size_t j = cap-1; j > i; j--) arr[j] = arr[j-1];
        arr[i].fps = fps;
        arr[i].avg = (double)avg_p;       arr[i].avg_prec = avg_p;
        arr[i].stddev = (double)stddev_p; arr[i].stddev_prec = stddev_p;
...
static void scan_range(double range_start, double range_end,
                       ...
                       BestResult **out_sweet, BestResult **out_jitter,
                       size_t *num_bins,
                       int integer_mode,
                       prec_t *err_buf, size_t max_capacity)
{
    ...
    if (full_window > total_samples) {
        ...
        *out_up = *out_down = *out_sweet = *out_jitter = NULL;
        *num_bins = 0;
        return;
    }
    ...
    BestResult *up = NULL, *down = NULL, *sweet = NULL, *jitter = NULL;
    size_t up_cnt = 0, down_cnt = 0, sweet_cnt = 0, jitter_cnt = 0;
    ...
    if (BINNING_MODE) {
        *num_bins = bins;
        ...
        jitter = xcalloc(bins, sizeof *jitter);
        ...
        *num_bins = TOP_N;
        ...
        jitter = xcalloc(TOP_N, sizeof *jitter);
    }
    ...
    if (g_GunzFdeltaMode) {
        if (integer_mode) {
            ...
            for (int fps_int = int_start; fps_int <= int_end; fps_int++) {
                ...
                prec_t mean_ms, stddev_ms, motion_jitter, score;
                compute_gunz_fdelta_metrics(cfps, &mean_ms, &stddev_ms, &motion_jitter, &score);
                ...
                if (BINNING_MODE) {
                    ...
                    if (!up[idx].valid || mean_ms < up[idx].avg_prec) {
                        ...
                        up[idx].motion_jitter = (double)motion_jitter;
                        up[idx].motion_jitter_prec = motion_jitter;
                    }
                    if (!down[idx].valid || mean_ms > down[idx].avg_prec) {
                        ...
                        down[idx].motion_jitter = (double)motion_jitter;
                        down[idx].motion_jitter_prec = motion_jitter;
                    }
                    if (!sweet[idx].valid || score < sweet[idx].sweet_prec) {
                        ...
                        sweet[idx].motion_jitter = (double)motion_jitter;
                        sweet[idx].motion_jitter_prec = motion_jitter;
                    }
                    if (!jitter[idx].valid || stddev_ms > jitter[idx].stddev_prec) {
                        jitter[idx].valid = 1; jitter[idx].fps = cfps;
                        jitter[idx].avg = (double)mean_ms;
                        jitter[idx].stddev = (double)stddev_ms;
                        jitter[idx].stddev_prec = stddev_ms;
                        jitter[idx].motion_jitter = (double)motion_jitter;
                        jitter[idx].motion_jitter_prec = motion_jitter;
                    }
                } else {
                    // Non‑binning top‑N
                    insert_down(up, TOP_N, &up_cnt, cfps, mean_ms, stddev_ms, motion_jitter);
                    insert_up(down, TOP_N, &down_cnt, cfps, mean_ms, stddev_ms, motion_jitter);
                    insert_sweet(sweet, TOP_N, &sweet_cnt, cfps, mean_ms, stddev_ms,
                                 (prec_t)0.0, score, motion_jitter);
                    insert_jitter(jitter, TOP_N, &jitter_cnt, cfps, mean_ms, stddev_ms, motion_jitter);
                }
            }
        } else {
        ...
        if (!BINNING_MODE) {
            *num_bins = TOP_N;
            ...
            for (size_t i = jitter_cnt; i < TOP_N; i++) jitter[i].valid = 0;
        }
        *out_up = up; *out_down = down; *out_sweet = sweet; *out_jitter = jitter;
        return;
    }

    // ===================== STANDARD MODE: INTEGER CENTRES (restored from 0.3.0) =====================
    if (integer_mode) {
        int int_start = (int)ceil(range_start);
        int int_end   = (int)floor(range_end);
        for (int fps_int = int_start; fps_int <= int_end; fps_int++) {
            double cfps = (double)fps_int;
            double centre_idx_d = (cfps - ext_start) / FPS_STEP;
            size_t ci = (size_t)llround(centre_idx_d);
            if (ci < half || ci >= total_samples - half) continue;

            // Kahan-compensated summation for precision matching decimal mode
            accum_t win_sum  = (accum_t)0.0, c_s  = (accum_t)0.0;
            accum_t win_sum2 = (accum_t)0.0, c_s2 = (accum_t)0.0;
            accum_t win_abs  = (accum_t)0.0, c_a  = (accum_t)0.0;
            size_t left  = ci - half;
            size_t right = ci + half;
            for (size_t j = left; j <= right; j++) {
                accum_t e = err_buf[j];
                kahan_add(&win_sum,  &c_s,  e);
                kahan_add(&win_sum2, &c_s2, e * e);
                kahan_add(&win_abs,  &c_a,  PREC_ABS(e));
            }

            accum_t inv_full = (accum_t)1.0 / (accum_t)full_window;
            accum_t avg_ld     = win_sum * inv_full;
            accum_t avg_abs_ld = win_abs * inv_full;
            accum_t var_ld     = (win_sum2 * inv_full) - (avg_ld * avg_ld);
            ...
            if (BINNING_MODE) {
                ...
                if (in_range) {
                    ...
                    if (!sweet[idx].valid || cheap_p < sweet[idx].sweet_prec) {
                        prec_t stddev_p = PREC_SQRT(var_ld);
                        prec_t sweet_p = (avg_abs_ld + (prec_t)SWEET_WEIGHT * stddev_p) * (prec_t)cfps;
                ...
                // jitter array remains invalid (no standard jitter analysis)
            } else {
                prec_t cheap_p = avg_abs_ld * (prec_t)cfps;
                if (avg_ld > (accum_t)0.0 && allow_for_extremes) {
                    prec_t stddev_p = PREC_SQRT(var_ld);
                    insert_up(up, TOP_N, &up_cnt, cfps, avg_ld, stddev_p, (prec_t)0.0);
                }
                if (avg_ld < (accum_t)0.0 && allow_for_extremes) {
                    prec_t stddev_p = PREC_SQRT(var_ld);
                    insert_down(down, TOP_N, &down_cnt, cfps, avg_ld, stddev_p, (prec_t)0.0);
                }
                if (in_range) {
                    int worth_computing = (sweet_cnt < TOP_N) ||
                        (sweet_cnt >= TOP_N && cheap_p < sweet[TOP_N-1].sweet_prec);
                    if (worth_computing) {
                        prec_t stddev_p = PREC_SQRT(var_ld);
                        prec_t sweet_p = (avg_abs_ld + (prec_t)SWEET_WEIGHT * stddev_p) * (prec_t)cfps;
                        insert_sweet(sweet, TOP_N, &sweet_cnt,
                                     cfps, avg_ld, stddev_p, avg_abs_ld, sweet_p, (prec_t)0.0);
                    }
                }
            }
        }
        if (!BINNING_MODE) {
            *num_bins = TOP_N;
            ...
            for (size_t i = down_cnt; i < TOP_N; i++) down[i].valid = 0;
            for (size_t i = sweet_cnt; i < TOP_N; i++) sweet[i].valid = 0;
            // jitter already all invalid
        }
        *out_up = up; *out_down = down; *out_sweet = sweet; *out_jitter = jitter;
        return;
    }

    // ===================== STANDARD DECIMAL MODE (unchanged except for sweet weight) =====================
...
// -----------------------------------------------------------------
// Merge and print (supports native mode formatting, with proper deduplication
// and HIGHEST MOTION JITTER section)
// -----------------------------------------------------------------
static void merge_and_print(BestResult **up_arrays,
                            BestResult **down_arrays,
                            BestResult **sweet_arrays,
                            BestResult **jitter_arrays,
                            size_t *bin_counts,
                            int num_ranges,
                            int integer_mode,
                            int binning_mode)
{
    ...
    for (int r = 0; r < num_ranges; r++)
        if (up_arrays[r] && down_arrays[r] && sweet_arrays[r] && jitter_arrays[r])
    ...
    RankResult  *jitter_r = malloc(total_bins * sizeof *jitter_r);
    if (!up_r || !down_r || !sweet_r || !jitter_r) { fprintf(stderr, "Memory error\n"); exit(1); }
    size_t up_n = 0, down_n = 0, sweet_n = 0, jitter_n = 0;

    for (int r = 0; r < num_ranges; r++) {
        BestResult *up = up_arrays[r], *down = down_arrays[r], *sweet = sweet_arrays[r], *jitter = jitter_arrays[r];
        if (!up || !down || !sweet || !jitter) continue;
        ...
        for (size_t i = 0; i < bins; i++) {
            if (up[i].valid) {
                ...
                up_r[up_n].motion_jitter = up[i].motion_jitter;
                up_r[up_n].motion_jitter_prec = up[i].motion_jitter_prec;
                up_n++;
            }
            if (down[i].valid) {
                ...
                down_r[down_n].motion_jitter = down[i].motion_jitter;
                down_r[down_n].motion_jitter_prec = down[i].motion_jitter_prec;
                down_n++;
            }
            if (sweet[i].valid) {
                ...
                sweet_r[sweet_n].motion_jitter = sweet[i].motion_jitter;
                sweet_r[sweet_n].motion_jitter_prec = sweet[i].motion_jitter_prec;
                sweet_n++;
            }
            if (jitter[i].valid) {
                jitter_r[jitter_n].fps = jitter[i].fps;
                jitter_r[jitter_n].avg = jitter[i].avg;   jitter_r[jitter_n].avg_prec = jitter[i].avg_prec;
                jitter_r[jitter_n].stddev = jitter[i].stddev; jitter_r[jitter_n].stddev_prec = jitter[i].stddev_prec;
                jitter_r[jitter_n].motion_jitter = jitter[i].motion_jitter;
                jitter_r[jitter_n].motion_jitter_prec = jitter[i].motion_jitter_prec;
                jitter_n++;
            }
        }
    }
    ...
    if (up_n > 0) {
        if (g_GunzFdeltaMode) {
            ...
            size_t display;
            if (binning_mode) {
                // Deduplicate using a seen array – keep only the first (best) occurrence per integer FPS
                int seen[1006] = {0};
                size_t dedup_n = 0;
                for (size_t k = 0; k < up_n && dedup_n < (size_t)TOP_N; ++k) {
                    int bin = (int)floor(up_r[k].fps);
                    if (bin >= 0 && bin <= 1005 && !seen[bin]) {
                        up_r[dedup_n++] = up_r[k];
                        seen[bin] = 1;
                    }
                }
                ...
                display = (size_t)TOP_N < up_n ? (size_t)TOP_N : up_n;
            }

            printf("=== FASTEST BLOCK (lower ms, native engine) ===\n");
            for (size_t i = 0; i < display; i++) {
                printf("%3zu. %10.5f FPS    block= %7.3f ms   jitter= %6.2f ms   motion_jitter= %6.2f ticks\n",
                       i+1, up_r[i].fps, up_r[i].avg, up_r[i].stddev, up_r[i].motion_jitter);
            }
        } else {
    ...
    if (down_n > 0) {
        if (g_GunzFdeltaMode) {
            ...
            if (binning_mode) {
                int seen[1006] = {0};
                size_t dedup_n = 0;
                for (size_t k = 0; k < down_n && dedup_n < (size_t)TOP_N; ++k) {
                    int bin = (int)floor(down_r[k].fps);
                    if (bin >= 0 && bin <= 1005 && !seen[bin]) {
                        down_r[dedup_n++] = down_r[k];
                        seen[bin] = 1;
                    }
                }
                display = dedup_n;
            } else {
                display = (size_t)TOP_N < down_n ? (size_t)TOP_N : down_n;
            }

            printf("\n=== SLOWEST BLOCK (higher ms, native engine) ===\n");
            for (size_t i = 0; i < display; i++) {
                printf("%3zu. %10.5f FPS    block= %7.3f ms   jitter= %6.2f ms   motion_jitter= %6.2f ticks\n",
                       i+1, down_r[i].fps, down_r[i].avg, down_r[i].stddev, down_r[i].motion_jitter);
            }
        } else {
    ...
    if (sweet_n > 0) {
        if (g_GunzFdeltaMode) {
            ...
            if (binning_mode) {
                int seen[1006] = {0};
                size_t dedup_n = 0;
                for (size_t k = 0; k < sweet_n && dedup_n < (size_t)TOP_N; ++k) {
                    int bin = (int)floor(sweet_r[k].fps);
                    if (bin >= 0 && bin <= 1005 && !seen[bin]) {
                        sweet_r[dedup_n++] = sweet_r[k];
                        seen[bin] = 1;
                    }
                }
                display = dedup_n;
            } else {
                display = (size_t)TOP_N < sweet_n ? (size_t)TOP_N : sweet_n;
            }

            printf("\n=== BEST ACCURACY (closest to %.1f ms %s, native engine) ===\n",
                   GUNZ_BLOCK_REF_MS,
                   BLOCK_METRIC_MODE ? "repeat interval" : "block");
            printf("(Lower score = %s dev. + %.1f * block_jitter + %.1f * motion_jitter)\n",
                   BLOCK_METRIC_MODE ? "interval" : "block time",
                   (double)GUNZ_JITTER_WEIGHT,
                   (double)SWEET_WEIGHT_MOTION);
            for (size_t i = 0; i < display; i++) {
                printf("%3zu. %10.5f FPS    block= %7.3f ms   jitter= %6.2f ms   "
                       "motion_jitter= %6.2f ticks   score= %6.1f\n",
                       i+1, sweet_r[i].fps, sweet_r[i].avg, sweet_r[i].stddev,
                       sweet_r[i].motion_jitter, sweet_r[i].sweet);
            }
        } else {
    ...
    // JITTER / HIGHEST JITTER
    if (jitter_n > 0) {
        if (g_GunzFdeltaMode) {
            qsort(jitter_r, jitter_n, sizeof *jitter_r, cmp_jitter_native);

            size_t display;
            if (binning_mode) {
                int seen[1006] = {0};
                size_t dedup_n = 0;
                for (size_t k = 0; k < jitter_n && dedup_n < (size_t)TOP_N; ++k) {
                    int bin = (int)floor(jitter_r[k].fps);
                    if (bin >= 0 && bin <= 1005 && !seen[bin]) {
                        jitter_r[dedup_n++] = jitter_r[k];
                        seen[bin] = 1;
                    }
                }
                display = dedup_n;
            } else {
                display = (size_t)TOP_N < jitter_n ? (size_t)TOP_N : jitter_n;
            }

            printf("\n=== HIGHEST JITTER (ms, native engine) ===\n");
            for (size_t i = 0; i < display; i++) {
                printf("%3zu. %10.5f FPS    jitter= %6.2f ms   block= %7.3f ms\n",
                       i+1, jitter_r[i].fps, jitter_r[i].stddev, jitter_r[i].avg);
            }

            // Now produce HIGHEST MOTION JITTER using the same array but sorted by motion_jitter
            if (jitter_n > 0) {
                qsort(jitter_r, jitter_n, sizeof *jitter_r, cmp_motion_jitter_native);

                size_t display_m;
                if (binning_mode) {
                    int seen_m[1006] = {0};
                    size_t dedup_m = 0;
                    for (size_t k = 0; k < jitter_n && dedup_m < (size_t)TOP_N; ++k) {
                        int bin = (int)floor(jitter_r[k].fps);
                        if (bin >= 0 && bin <= 1005 && !seen_m[bin]) {
                            jitter_r[dedup_m++] = jitter_r[k];
                            seen_m[bin] = 1;
                        }
                    }
                    display_m = dedup_m;
                } else {
                    display_m = (size_t)TOP_N < jitter_n ? (size_t)TOP_N : jitter_n;
                }

                printf("\n=== HIGHEST MOTION JITTER (ticks, native engine) ===\n");
                for (size_t i = 0; i < display_m; i++) {
                    printf("%3zu. %10.5f FPS    motion_jitter= %6.2f ticks   block= %7.3f ms\n",
                           i+1, jitter_r[i].fps, jitter_r[i].motion_jitter, jitter_r[i].avg);
                }
            }
        } else {
            printf("\nNo JITTER results found.\n");
        }
    }

    free(up_r); free(down_r); free(sweet_r); free(jitter_r);
}

// -----------------------------------------------------------------
// Single‑point mode (restored from 0.3.0)
// -----------------------------------------------------------------
...
void run_single_point_mode(void) {
    ...
    if (!(FPS_STEP > 0.0)) { fprintf(stderr, "Error: FPS_STEP > 0\n"); exit(1); }

    double max_fps = 0.0;
    if (BATCH_MODE) {
        for (int i = 0; i < NUM_BATCH_RANGES; i++)
            if (batch_ends[i] > max_fps) max_fps = batch_ends[i];
    } else {
        max_fps = FPS_END;
    }
    size_t array_size = (size_t)ceil(max_fps) + 10;
    if (array_size < 1005) array_size = 1005;

    double *best_up_fps = malloc(array_size * sizeof *best_up_fps);
    prec_t *best_up_err = malloc(array_size * sizeof *best_up_err);
    int    *valid_up    = calloc(array_size, sizeof *valid_up);
    double *best_down_fps = malloc(array_size * sizeof *best_down_fps);
    prec_t *best_down_err = malloc(array_size * sizeof *best_down_err);
    int    *valid_down    = calloc(array_size, sizeof *valid_down);
    double *zero_fps = malloc(TOP_N * sizeof *zero_fps);
    prec_t *zero_abs = malloc(TOP_N * sizeof *zero_abs);
    if (!best_up_fps || !best_up_err || !valid_up ||
        !best_down_fps || !best_down_err || !valid_down ||
        !zero_fps || !zero_abs) {
    ...
    for (size_t i = 0; i < array_size; i++) {
        best_up_err[i] = -prec_inf();
        best_down_err[i] = prec_inf();
    }
    int zero_cnt = 0;
    ...
    printf("Single‑point extreme scan near %.0f FPS (range ±%.0f, direction %s)\n",
           TARGET_FPS, TARGET_RANGE,
           TARGET_DIRECTION == 0 ? "both" : (TARGET_DIRECTION == 1 ? "≤ target" : "≥ target"));

    for (size_t r = 0; r < (BATCH_MODE ? NUM_BATCH_RANGES : 1); r++) {
        double start = BATCH_MODE ? batch_starts[r] : FPS_START;
        double end   = BATCH_MODE ? batch_ends[r]   : FPS_END;

        if (INTEGER_MODE) {
            for (int int_fps = (int)start; int_fps <= (int)end; int_fps++) {
                double fps = (double)int_fps;
                if (!fps_in_target(fps)) continue;
                prec_t error = compute_timing_error(fps);
                int idx = int_fps;
                if (idx < 0 || (size_t)idx >= array_size) continue;
                if (error > best_up_err[idx]) {
                    best_up_err[idx] = error; best_up_fps[idx] = fps; valid_up[idx] = 1;
                }
                if (error < best_down_err[idx]) {
                    best_down_err[idx] = error; best_down_fps[idx] = fps; valid_down[idx] = 1;
                }
                insert_sorted_zero(fps, PREC_ABS(error), zero_fps, zero_abs, &zero_cnt, TOP_N);
            }
        } else {
            if ((end - start) / FPS_STEP > (double)SIZE_MAX) {
                fprintf(stderr, "Error: Range too large for single-point scan.\n");
                exit(1);
            }
            size_t n_samples = (size_t)((end - start) / FPS_STEP) + 1;
            for (size_t i = 0; i < n_samples; i++) {
                double fps = start + (double)i * FPS_STEP;
                if (!fps_in_target(fps)) continue;
                prec_t error = compute_timing_error(fps);
                int idx = (int)llround(fps);
                if (idx < 0 || (size_t)idx >= array_size) continue;
                if (error > best_up_err[idx]) {
                    best_up_err[idx] = error; best_up_fps[idx] = fps; valid_up[idx] = 1;
                }
    ...
    UpDownItem *up_items = malloc(array_size * sizeof *up_items);
    if (up_items) {
        int up_count = 0;
        for (size_t i = 0; i < array_size; i++)
            if (valid_up[i]) { up_items[up_count].fps = best_up_fps[i]; up_items[up_count].err = best_up_err[i]; up_count++; }
        if (up_count > 0) {
            qsort(up_items, up_count, sizeof *up_items, cmp_up_item);
            size_t display = (size_t)TOP_N < (size_t)up_count ? (size_t)TOP_N : (size_t)up_count;
            printf("\n=== TOP %zu UP (best per integer, largest positive error) ===\n", display);
            for (size_t i = 0; i < display; i++) {
                printf("%3zu. %10.5f FPS    error = ", i+1, up_items[i].fps);
#if ACTUAL_PRECISION == 2
                char buf[128];
                quadmath_snprintf(buf, sizeof buf, "%+46.36Qe", up_items[i].err);
                printf("%s\n", buf);
#elif ACTUAL_PRECISION == 1
                printf("%+38.21Le\n", (long double)up_items[i].err);
#else
                printf("%+20.6e\n", (double)up_items[i].err);
#endif
            }
        }
        free(up_items);
    ...
    UpDownItem *down_items = malloc(array_size * sizeof *down_items);
    if (down_items) {
        int down_count = 0;
        for (size_t i = 0; i < array_size; i++)
            if (valid_down[i]) { down_items[down_count].fps = best_down_fps[i]; down_items[down_count].err = best_down_err[i]; down_count++; }
        if (down_count > 0) {
            qsort(down_items, down_count, sizeof *down_items, cmp_down_item);
            size_t display = (size_t)TOP_N < (size_t)down_count ? (size_t)TOP_N : (size_t)down_count;
            printf("\n=== TOP %zu DOWN (best per integer, most negative error) ===\n", display);
            for (size_t i = 0; i < display; i++) {
                printf("%3zu. %10.5f FPS    error = ", i+1, down_items[i].fps);
#if ACTUAL_PRECISION == 2
                char buf[128];
                quadmath_snprintf(buf, sizeof buf, "%+46.36Qe", down_items[i].err);
                printf("%s\n", buf);
#elif ACTUAL_PRECISION == 1
                printf("%+38.21Le\n", (long double)down_items[i].err);
#else
                printf("%+20.6e\n", (double)down_items[i].err);
#endif
            }
        }
        free(down_items);
    ...
    printf("\n=== TOP %d CLOSEST TO ZERO (global) ===\n", TOP_N);
    if (zero_cnt == 0) {
        printf("No near‑zero values found.\n");
    } else {
        size_t display = (size_t)TOP_N < (size_t)zero_cnt ? (size_t)TOP_N : (size_t)zero_cnt;
        for (size_t i = 0; i < display; i++) {
            prec_t raw_err = compute_timing_error(zero_fps[i]);
            printf("%3zu. %10.5f FPS    error = ", i+1, zero_fps[i]);
#if ACTUAL_PRECISION == 2
            char buf1[128], buf2[128];
            quadmath_snprintf(buf1, sizeof buf1, "%+46.36Qe", raw_err);
            quadmath_snprintf(buf2, sizeof buf2, "%46.36Qe",  zero_abs[i]);
            printf("%s  abs = %s%s\n", buf1, buf2,
                   zero_abs[i] == 0 ? " *** EXACT ZERO ***" : "");
#elif ACTUAL_PRECISION == 1
            printf("%+38.21Le  abs = %38.21Le%s\n", (long double)raw_err,
                   (long double)zero_abs[i],
                   zero_abs[i] == 0 ? " *** EXACT ZERO ***" : "");
#else
            printf("%+20.6e  abs = %20.6e%s\n", (double)raw_err,
                   (double)zero_abs[i],
                   zero_abs[i] == 0.0 ? " *** EXACT ZERO ***" : "");
#endif
        }
    }

    free(best_up_fps); free(best_up_err); free(valid_up);
    free(best_down_fps); free(best_down_err); free(valid_down);
    free(zero_fps); free(zero_abs);
}
...
void run_decimal_mode(double start, double end) {
    ...
    size_t num_bins;
    BestResult *up, *down, *sweet, *jitter;
    scan_range(start, end, &up, &down, &sweet, &jitter, &num_bins, 0, err_buf, n_max);

    // Guard against NULL arrays (window too large for range)
    if (!up || !down || !sweet || !jitter) {
        fprintf(stderr, "Error: scan_range failed (range too small?)\n");
    ...
    BestResult *jitter_arr[1] = { jitter };
    merge_and_print(up_arr, down_arr, sweet_arr, jitter_arr, &num_bins, 1, 0, BINNING_MODE);
    free(up); free(down); free(sweet); free(jitter);
    if (err_buf) free(err_buf);
}
...
void run_integer_mode(double start, double end) {
    ...
    size_t num_bins;
    BestResult *up, *down, *sweet, *jitter;
    scan_range(start, end, &up, &down, &sweet, &jitter, &num_bins, 1, err_buf, n_max);
    ...
    BestResult *sweet_arr[1] = { sweet };
    BestResult *jitter_arr[1] = { jitter };
    merge_and_print(up_arr, down_arr, sweet_arr, jitter_arr, &num_bins, 1, 1, BINNING_MODE);
    free(up); free(down); free(sweet); free(jitter);
    if (err_buf) free(err_buf);
}
...
int main(void) {
    ...
    if (BATCH_MODE) {
        ...
        BestResult *jitter_arrays[NUM_BATCH_RANGES] = {NULL};
...
#pragma omp parallel for schedule(dynamic)
        ...
        for (size_t r = 0; r < NUM_BATCH_RANGES; r++) {
            ...
            scan_range(batch_starts[r], batch_ends[r],
                       &up_arrays[r], &down_arrays[r], &sweet_arrays[r], &jitter_arrays[r],
        ...
        merge_and_print(up_arrays, down_arrays, sweet_arrays, jitter_arrays,
                        bin_counts, NUM_BATCH_RANGES, INTEGER_MODE, BINNING_MODE);
        for (size_t r = 0; r < NUM_BATCH_RANGES; r++) {
            if (up_arrays[r])   free(up_arrays[r]);
            if (down_arrays[r]) free(down_arrays[r]);
            if (sweet_arrays[r]) free(sweet_arrays[r]);
            if (jitter_arrays[r]) free(jitter_arrays[r]);
        }
